    ...
    <table class="table1">
      ...
      <tbody>
        <tr v-for="(row, index) in table1Data" :key="index">
          ...
          <td>
            <select v-model="row.remarks" @change="updateRemark(index, row.remarks)">
              <option value="">Remarks</option>
              <option v-for="option in remarksOptions" :value="option.value">{{ option.label }}</option>
            </select>
            <br>
      ...
      <tbody>
        <tr v-for="(row, index) in table2Data" :key="index">
          <td>
            ...
            <textarea v-model="row.status" rows="3"></textarea>
          </td>
          <td>
            <label>Remarks:</label><br>
            <textarea v-model="row.recommendations" rows="3" style="color: black;" wrap="off"></textarea> <!-- Disable text wrapping -->
